...
int chooseFive=4;
int chooseSix=5;
forMsgQueue messageTemp;
forMsgQueue messageReal;

key_t forKeyMsgId=1234;
int toplam=0;
static void Signal_Handler(int signo){
	...
	gecenTime=(double)(bitis-baslangic) /CLOCKS_PER_SEC;
	 fprintf(stderr, "CTRL-C GELDİ!!PROGRAMDAN CİKİLDİ");
	if(signo==SIGINT){
		 while (r_wait(NULL) > 0); 
    ...
    fprintf(stderr, "Number of shared memories: %d\n", olusanKlasor);
    while(counter<olusanKlasor){

        fprintf(stderr, "Area of shared memories: %d\n", sizeof(int));
        ++counter;
    }
...
int main(int argc, char** argv) {
    ...
    gecenTime=(double)(bitis-baslangic)/ CLOCKS_PER_SEC;
     ...
     MessageSameID=msgget(forKeyMsgId,0666);

     if(MessageSameID<0){


        fprintf(stderr, "   MSGGET HATA!!\n");
    ...
    for(counter=0;counter<olusanKlasor ;++counter){


    		msgrcv(MessageSameID,&messageTemp,PATH_UZUNLUK,1,0);

    		a=atoi(messageTemp.myArray);
    ...
    fprintf(inp, "%d %s were found in total.", toplam,argv[1]);
    fclose(inp);
...
void ReadStringFromDirectories(char *gelenString, char *directory) {
	...
	/*shared memory için değişkenler */
	int shrdMemoryID=0;
	int sharedmemflag=IPC_CREAT | 0666;
	int *forSharedMemory;
	/*------------------------------*/

	/* Message Queue icin değişkenler */
    ...
    shrdMemoryID=shmget(IPC_PRIVATE,sizeof(int),sharedmemflag);

    if(shrdMemoryID<0){

    	perror("smget ID hata!!!!!");
    	exit(ZERO);
    }

    forSharedMemory= (int*)shmat(shrdMemoryID,NULL,0);

    if((int)*forSharedMemory<0){

    		fprintf(stderr,"shmat hata!!!!!\n");
    		exit(ZERO);

    }

   messageReal.myType=1;
   forMsgQueueId=msgget(forKeyMsgId,IPC_CREAT | 0666);
   ...
   if(forMsgQueueId<0){
    ...
    while (r_wait(NULL) > 0);  
    *forSharedMemory=eslesenDegisken;

    sprintf(messageReal.myArray,"%d",*forSharedMemory);

    messageSize =strlen(messageReal.myArray)+1;
   	kontrol=msgsnd( forMsgQueueId, &messageReal,messageSize, IPC_NOWAIT);
   	if(kontrol<0){

   		fprintf(stderr, "MSGSEND HATALI!!\n");
   		exit(ZERO);

   	}

   	shmdt(forSharedMemory);
   	shmctl(shrdMemoryID,IPC_RMID,ZERO);
